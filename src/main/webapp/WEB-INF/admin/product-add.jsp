...
<div style="padding:10px 10px 10px 10px">
	<form id="itemAddForm" class="itemForm" method="post">
	    <table cellpadding="5">
	        ...
	        <tr>
	            <td>商城价:</td>
	            <td><input class="easyui-numberbox" type="text" name="price2View" data-options="min:1,max:99999999,precision:2,required:true" />
	            	<input type="hidden" name="shopPrice"/>
	            </td>
	        </tr>
	        <tr>
	            <td>库存数量:</td>
	            <td><input class="easyui-numberbox" type="text" name="count" data-options="min:1,max:99999999,precision:0,required:true" /></td>
	        </tr>
	        <tr>
	            <td>是否热门:</td>
	            <td>
...
<script type="text/javascript">
	...
	$(function(){
		//创建富文本编辑器
		itemAddEditor = TAOTAO.createEditor("#itemAddForm [name=pDesc]");
		//初始化类目选择和图片上传器
	...
	function submitForm(){
		...
		if(!$('#itemAddForm').form('validate')){
		 ...
		 //取商品价格
		$("#itemAddForm [name=marketPrice]").val(eval($("#itemAddForm [name=priceView]").val()));
		$("#itemAddForm [name=shopPrice]").val(eval($("#itemAddForm [name=price2View]").val()));
		...
		*/
		...
		$.post("${pageContext.request.contextPath}/adminProduct/addProduct",$("#itemAddForm").serialize(), function(data){
			if(data=="ok"){
				$('#itemAddForm').form('reset');
				itemAddEditor.html('');
				//重置图片上传器
				TAOTAO.initEdit({fun:function(node){
